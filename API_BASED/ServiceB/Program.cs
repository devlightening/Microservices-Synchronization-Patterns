    ...
    MongoDBService mongoDBService) =>
{
    ...
    // Eðer çalýþan bulunamazsa (employee null ise), bir hata mesajý veya null dönebilirsiniz.
    if (employee is null)
    {
        return Results.NotFound($"Employee with PersonId {personId} not found.");
    }

    employee.Name = newName;
    await employees.FindOneAndReplaceAsync(p => p.Id == employee.Id, employee);
    return Results.Ok(true); // Ýþlem baþarýlý olduðunda OK (200) dönebilirsiniz
});
